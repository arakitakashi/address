...
public record Prefecture(String value) {
    ...
    public Prefecture {
        if (isBlank(value)) {
            throw new IllegalArgumentException(PREFECTURE_NOT_BLANK_MESSAGE.getMessage());
        }
        if (value.length() > 20) {
            throw new IllegalArgumentException(PREFECTURE_MAX_20_DIGITS_MESSAGE.getMessage());
        }
    }
}

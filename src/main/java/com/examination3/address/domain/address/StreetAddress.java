...
public record StreetAddress(String value) {
    ...
    public StreetAddress {
        if (isBlank(value)) {
            throw new IllegalArgumentException(STREET_ADDRESS_NOT_BLANK_MESSAGE.getMessage());
        }
        if (value.length() > 100) {
            throw new IllegalArgumentException(STREET_ADDRESS_MAX_100_DIGITS_MESSAGE.getMessage());
        }
    }
}
